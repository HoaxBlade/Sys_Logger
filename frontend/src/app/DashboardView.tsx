        ...
        <div className="h-screen overflow-hidden bg-[#FAFAFA] text-zinc-900 font-sans flex flex-col p-2 sm:p-4 lg:p-6 gap-4 lg:gap-6 relative selection:bg-orange-500/20">
            ...
            <div className="flex flex-1 overflow-hidden gap-6 relative z-10 min-w-0 min-h-0">
                ...
                <aside className={cn(
                        ...
                        <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-4 px-1 min-h-[48px]">
                            {(['all', 'connected', 'warning', 'critical', 'disconnected'] as const).map((status) => (
                                <button
                                    ...
                                    className={cn(
                                        "px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest whitespace-nowrap transition-all ring-1 hover:scale-[1.02] active:scale-95",
                                        filterStatus === status
                    ...
                    <div className="flex-1 overflow-y-auto p-4 lg:p-5 space-y-8 no-scrollbar bg-white/5 relative mt-4">
                        <div className="absolute inset-x-0 top-0 h-12 bg-gradient-to-b from-white/40 to-transparent pointer-events-none z-10" />
                            ...
                            Object.entries(groupedUnits).map(([org, orgUnits]) => (
                                <div key={org} className="space-y-3">
                                    <button 
                                        ...
                                        className="flex items-center gap-3 px-1 mb-5 group/header w-full hover:opacity-80 transition-opacity mt-4"
                                    >
                                                    ...
                                                    {orgUnits.map((unit) => {
                                                        ...
                                                        return (
                                                            <motion.div
                                                                ...
                                                                className={cn(
                                                                    "w-full text-left p-4 rounded-[1.5rem] transition-all duration-300 relative group/card cursor-pointer border-2",
                                                                    isSelected
                                                                        ? 'bg-white shadow-[0_12px_30px_rgba(249,115,22,0.12)] border-orange-500 scale-[1.02]'
                                                                        : 'hover:bg-white hover:shadow-xl hover:shadow-zinc-900/5 border-transparent'
                                                                )}
                                                            >
                        ...
                        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                            ...
                            >
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-5 bg-orange-50/50 rounded-2xl border border-orange-100 flex flex-col gap-4">
                                        <div className="flex items-center gap-3">
                                            <div className="p-1.5 bg-orange-500 rounded-lg shadow-sm">
                                                <Zap className="w-3 h-3 text-white" />
                                            </div>
                                            <span className="text-[10px] font-black uppercase tracking-widest text-orange-600">Installation Steps</span>
                                        </div>
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-3">
                                                <div className="w-5 h-5 rounded-full bg-zinc-900 text-white flex items-center justify-center text-[9px] font-black">1</div>
                                                <p className="text-[11px] text-zinc-600 font-bold">Download the specialized installer bundle.</p>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <div className="w-5 h-5 rounded-full bg-zinc-900 text-white flex items-center justify-center text-[9px] font-black">2</div>
                                                <p className="text-[11px] text-zinc-600 font-bold">Unship (unzip) it directly into your <code className="text-orange-600">C:\</code> folder.</p>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <div className="w-5 h-5 rounded-full bg-zinc-900 text-white flex items-center justify-center text-[9px] font-black">3</div>
                                                <p className="text-[11px] text-zinc-600 font-bold">Open the folder and run <code className="text-orange-600">install.bat</code> as Admin.</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {generatedLink ? (
                                        <div className="flex flex-col gap-3">
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                <a 
                                                    href={generatedLink}
                                                    download={`sys_logger_installer_${newNodeName}.zip`}
                                                    className="w-full bg-emerald-600 text-white rounded-2xl py-4 font-black uppercase tracking-widest text-[11px] flex items-center justify-center gap-2 hover:bg-emerald-500 transition-all shadow-md"
                                                >
                                                    Download Now
